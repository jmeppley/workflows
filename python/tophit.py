...
def get_top_hit_outputs(config):
    ...
    if len(samples) == 0:
        raise Exception(
            "Please supply a list of samples and reads in "
    ...
    try:
        ...
        raise Exception(
            "There must be a map of databases called 'dbs' in " "your configuration!"
        )
    ...
    for sample in samples:
            ...
            if isinstance(reads_file[0], str):
                ...
                raise Exception(
                    (
                        ...
                        "{reads_file}"
                    ).format(sample=sample, reads_file=repr(reads_file))
                )
        ...
        if is_prot is None:
            ...
            if is_prot != this_file_is_prot:
                raise Exception(
                    "All fasta files must be either nucl or AA. "
    ...
    for dbase in dbs:
        ...
        db_strings[dbase] = "vs.{dbase}.{search_alg}.{hit_filter}".format(**vars())
        ...
        if config.get("remove_rna"):
            ...
            config["outputs"].add(
                "counts.{dbase}.{topalg}.{rrna_subset}.hitids".format(**vars())
            )
...
def get_filter_string(filter_dict):
    ...
    """
    return "".join(
        "_{}{}".format(k, filter_dict[k]) for k in sorted(filter_dict.keys())
    )
