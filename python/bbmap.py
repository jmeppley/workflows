...
def get_bbduk_resource_path(config):
    ...
    try:
        cmd = (
            "find `which bbduk.sh | sed s/bbduk\.sh/../` "
            + '-name "adapters.fa" | sed s#/adapters.fa##'
        )
        resource_path = subprocess.check_output(cmd, shell=True).decode().strip()
    except:
        print(
            "Can't find bbmap resources, please set system_paths->bbmap in the snakemake config file"
        )
        raise
    else:
        return resource_path
