...
def get_annot_coverage_stats(contig_depth_file, annot_gff_file, annot_cov_out):
    """
        ...
        with open(contig_depth_file) as depth_handle:
            ...
            for contig, depths in contig_depths:
                try:
                    ...
                    # no genes for contig
                    continue
                contig_extent = max(max(g[1:]) for g in contig_genes)
                coverage = numpy.array(list(_insert_zeros(depths, contig_extent)))
...
def get_samtool_depth_table(depth_file, fasta_file=None):
    ...
    Returns:
     pandas.DataFrame with one row per contig and the following columns:
            ...
            """
    contig_lengths = (
        {r.id: len(r) for r in SeqIO.parse(fasta_file, "fasta")}
...
def get_contig_coverage_table(contig_depths_handle, contig_lengths=None):
    """
    ...
    table = pandas.DataFrame(
        _contig_cov_row_generator(contig_depths_handle, contig_lengths),
...
def _contig_cov_row_generator(contig_depths_handle, contig_lengths):
    ...
    for contig, depths in depths_iterator:
        coverage = numpy.array(
            list(_insert_zeros(depths, contig_lengths.pop(contig, 0)))
        )
        yield (
            contig,
