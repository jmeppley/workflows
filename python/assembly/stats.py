...
def get_contig_stats(contigs_fasta, contig_stats_out=None):
    """
...
def get_sequence_stats_from_contigs(contigs_fasta):
    ...
    return pandas.DataFrame(
        {"Contig": contigs, "Length": lengths, "GC": gcs}
    ).set_index("Contig")
...
def generate_histogram(
    ...
    **kwargs
    ...
    values = stats_table.query("Length >= " + str(length_cutoff))[metric].fillna(0)
    with open(histogram_file, "wt") as hist_handle:
        if values.empty:
            hist_handle.write(
                "There are no contigs longer than " "{length_cutoff}bp".format(**vars())
            )
        else:
            hist_handle.write(
    ...
    """.format(
                    ...
                    histogram=ascii_histogram(
                        numpy.histogram(values, **kwargs), log=log, width=txt_width
                    ),
                )
            )
...
def get_contig_length_summary_stats(contig_stats, N_levels=[50, 75, 90]):
    ...
    N_stats = {
        "contig count": contig_stats.shape[0],
