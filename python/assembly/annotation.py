...
def gff_hit_iterator(gff_file, **kwargs):
    ...
    for contig, hits in generate_hits(gff_file, format=GFF, **kwargs):
        for hit in hits:
            yield hit
...
RNA_GENE_REXP = re.compile(r"^(\S+)_[a-z]+RNA_\d+\s.+start=(\d+);end=(\d+);")
...
def drop_all_rna_overlaps(cds_files, rna_files, output_files, buffer):
    ...
    for input_file in cds_files:
        ...
        output_file = [f for f in output_files if f.endswith(input_file[-4:])][0]
        drop_rna_overlaps(input_file, output_file, rna_locations, buffer=buffer)
...
def filter_annotations(good_contig_file, input_file, output_file):
    ...
    logger.debug("Kept {} of {} genes from {}".format(out_genes, in_genes, input_file))


def drop_rna_overlaps(input_file, output_file, rna_locations, buffer=0):
    """
...
def filter_and_extract_rRNA(
    raw_gff, contigs_fasta, output_rna_fna, output_rna_gff, molecule, buffer=0
):
    """
    Take raw GFF of rRNA hits, and output GFF and fna of non-overlapping
    """
    filter_args = {"sort": "score", "nonoverlapping": buffer}
    gff_hits = {
        c: list(h) for c, h in generate_hits(raw_gff, format=GFF, **filter_args)
    }
        ...
        with open(output_rna_gff, "wt") as gff_out_handle:
            ...
            for contig in SeqIO.parse(contigs_fasta, "fasta"):
                ...
                if m:
                    ...
                    contig_info_string = "contig_length={};contig_cov={}".format(
                        length, coverage
                    )
                else:
                    # megahit
                ...
                for i, hit in enumerate(gff_hits.get(contig.id, [])):
                    source, feature_type, start, end, score, strand = hit.line.split(
                        "\t"
                    )[1:7]

                    # name gene with contig name and index
                    gene_name = contig.id + "_{mol}_{n}".format(n=i + 1, mol=molecule)
                    ...
                    if hit.qstart <= hit.qend:
                        ...
                        subsequence = contig.seq[
                            hit.qend - 1 : hit.qstart
                        ].reverse_complement()
                    if strand == "-":
                        subsequence = subsequence.reverse_complement()

                    fna_out_handle.write(
                        ">{seqid}\t{desc}\n{seq}\n".format(
                            seqid=gene_name, desc=gene_desc, seq=str(subsequence)
                        )
                    )
