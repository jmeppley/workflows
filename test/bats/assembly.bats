...
@test "assemble single sample with spades" {
    ...
    [ "$status" -eq 0 ]
    run bash -c "snakemake -j 10 -s ../../../assembly.metagenomic.snake -p --configfile ../../data/configs/spades.yaml -n 2>&1 | grep '^Nothing to be done'"
    [ "$status" -eq 0 ]
...
@test "assemble single sample with megahit" {
    ...
    [ "$status" -eq 0 ]
    run bash -c "snakemake -j 10 -s ../../../assembly.metagenomic.snake -p --configfile ../../data/configs/megahit.yaml -n 2>&1 | grep '^Nothing to be done'"
    [ "$status" -eq 0 ]
    [ "${lines[0]}" == "Nothing to be done." ]
}
...
@test "Prep two pair of files for assembly" {
    ...
    [ "$status" -eq 0 ]
    run bash -c "snakemake -j 10 -s ../../../clean.illumina.snake -p --configfile ../../data/configs/illumina.yaml --config cleaning_protocol=assembly remove_rna=False discover_fastx_for_stats=True -n 2>&1 | grep '^Nothing to be done'"
    [ "$status" -eq 0 ]
    [ "${lines[0]}" == "Nothing to be done." ]
}
