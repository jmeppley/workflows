...
@test "Prep two pair of scriptseq files with flash" {
    run bash -c "snakemake -s ../../../clean.illumina.snake -p --configfile ../../data/configs/illumina.yaml --verbose --config joining_program=flash run_stats=True > flash.join.log 2>&1"
    [ "$status" -eq 0 ]
}

@test "Prep two pair of scriptseq files with pandaseq" {
    which pandaseq || skip "Pandaseq is not installed"
    run bash -c "snakemake -s ../../../clean.illumina.snake -p --configfile ../../data/configs/illumina.yaml --verbose --config joining_program=pandaseq > pandaseq.join.log 2>&1"
    [ "$status" -eq 0 ]
}
